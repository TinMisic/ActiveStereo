...
right = camera.Camera("camera_parameters/camera1_intrinsics.dat",4)
    ...
    while True:
        ...
        if state=='init':
            ...
            if centroidL!=(None, None) and centroidR!=(None, None):
                ...
                max_dif = max(rel_ang, key = lambda x:abs(x)) # get max servo shift
                ...
                for i in range(len(servos)):
                    servos[i].write((angles+offsets[i])*scaling)
